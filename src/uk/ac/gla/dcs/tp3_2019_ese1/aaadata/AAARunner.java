...
public class AAARunner {
	...
	public void runTest(ActionEvent evt) {
		try {
			_board.enableEvent(EventType.ON_END_OF_INPUT_SCAN, (b, t, d) -> {
				...
				double[] acceleration = Arrays.stream(raw).mapToDouble(r -> SCALING_5V * (r - offset) / volts_per_g)
						.toArray();
	...
	private static double[] applyLegacyFilter(double[] acc) {
		...
		for (int i = 0; i < acc.length - 1; i++) {
			x = acc[i];
			acc[i] = (acc[i + 1] + 2 * x + prevx) / 1.777837895e+04F + 1.9786749573F * y - 0.9788999497F * prevy;
			prevx = x;
			prevy = y;
			y = acc[i];
		}
	 ...
	 * 
	 * @param acceleration - the array of acceleration samples (in g's), after
	 *                     filtering and conversion.
	 */
	private void analyseResults(double[] acceleration) {
		int drop_start = SAFETY_MARGIN;
		while (drop_start < acceleration.length && acceleration[drop_start] >= 0.0)
			drop_start++;

		int drop_final = drop_start;
		while (drop_final < (acceleration.length - SAFETY_MARGIN) && acceleration[drop_final] < 0.5)
			drop_final++;
		while (drop_final < (acceleration.length - SAFETY_MARGIN) && acceleration[drop_final] > 0.1)
			drop_final++;
		...
		if (v1 == 0.0)
			v1 = 99.9; /* XXX -- to avoid divide-by-zero I think? */
