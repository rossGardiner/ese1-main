...
public class LibcbwBoard implements AutoCloseable {
	 ...
	 * 
	 * @param item - one of the constants specified in {@link LibcbwJNA.ConfigInfo}
	 * @param devNum - an extra parameter whose significance depends on the item chosen.
	...
	public void setBoardInfoString(int item, int devNum, String value) throws LibcbwException {
		int err = LibcbwJNA.cbSetConfigString(ConfigInfo.BOARDINFO, _boardnum, devNum, item, value, new IntByReference(value.length()));
		if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err);
	}
	 ...
	 * @author Duncan Lowther (2402789L)
	 *
	 * Represents a specific connected USB-1608FS board. The board has the following specs:<br>
	 * <br>
	 * ===Analogue Input===<br>
	 * Functions : cbAIn(), cbAInScan(), cbALoadQueue(), cbFileAInScan(), cbATrig()<br>
	 * Options : BACKGROUND, BLOCKIO, BURSTIO(Count < 32768), CONTINUOUS, CONVERTDATA, EXTCLOCK,
	 * 			 EXTTRIGGER, NOCALIBRATEDATA, SINGLEIO<br>
	 * Mode : Single-ended<br>
	 * Channels : 0 to 7<br>
	 * Count : in BURSTIO, divisible by number of channels<br>
	 * Rate : 1 Hz - 50 kHz per channel, max 100 kHz over all channels (200 kHz for BURSTIO)<br>
	 ...
	 * <br>
	 * ===Digital I/O===<br>
	 * Functions : cbDConfigBit(), cbDConfigPort(), cbDOut(), cbDIn(), cbDBitIn(), cbDBitOut()<br>
	 * Port : AUXPORT<br>
	 * Bits : 0 to 7<br>
	 * <br>
	 * ===Counter I/O===<br>
	 ...
	 * Event Types : ON_SCAN_ERROR, ON_DATA_AVAILABLE, ON_END_OF_INPUT_SCAN<br>
	 * <br>
	 * ==Miscellaneous===<br>
	...
	public static class USB_1608FS extends LibcbwBoard {
		 ...
		 * 
		 * @param boardnum - the numeric ID of the board as assigned by InstaCal
		 * @throws LibcbwException if the board cannot be accessed, or if the board is not an USB-1608FS.
		 */
		 ...
		 * @param range - the voltage range for the reading. Must be one of the following
		 * 				  constants specified in {@link LibcbwJNA.ADCRange}:
		 *                BIP1VOLTS, BIP2VOLTS, BIP5VOLTS, or BIP10VOLTS
		 * @return the 16-bit unsigned measurement, zero-extended to fit an <code>int</code>
		 * @throws LibcbwException if an error occurs.
		 */
		...
		/**
		 * Read at regular intervals from one or more the ADC channels and return the results.
		 * 
		 * @param lowchan - the ADC channel number of the first channel to poll (0 to 7, inclusive)
		 ...
		 * @param freq - the sampling frequency (Hz) for each individual channel. Must be between
		 *               1 and 50000, inclusive, and <code>freq*numChannels</code> should not exceed
		 *               200000.
		 * @param options - a bitwise-OR combination of zero or more option flags from amongst
		 *                  the following list of constance specified in {@link LibcbwJNA.CBWOptions}:
		 *                  BLOCKIO, BURSTIO, EXTCLOCK, EXTTRIGGER, NOCALIBRATEDATA, SINGLEIO.
		 *                  Note that BLOCKIO, BURSTIO, and SINGLEIO are mutually exclusive and that if
		 *                  BLOCKIO is specified, <code>numChannels*samplesPerChannel</code> must not exceed
		 *                  32768.
		 ...
		 * @throws LibcbwException if an error occurs.
		 */
		public int[][] analogueInScan(int lowchan, int numChannels, int range, int samplesPerChannel, int freq, int options) throws LibcbwException {
			...
			int err = LibcbwJNA.cbAInScan(_boardnum, lowchan, lowchan + numChannels - 1, lcount, rateref, range, memhandle, options);
		...
		/**
		 * Begin an asynchronous scan, reading at regular intervals from one or more the ADC channels.
		 * 
		 * @param lowchan - the ADC channel number of the first channel to poll (0 to 7, inclusive)
		 * @param numChannels - the number of consecutive channels to poll
		 * @param range - the voltage range for the reading. Must be one of the following
		 ...
		 * @param options - a bitwise-OR combination of zero or more option flags from amongst
		 *                  the following list of constance specified in {@link LibcbwJNA.CBWOptions}:
		 *                  BLOCKIO, BURSTIO, CONTINUOUS, EXTCLOCK, EXTTRIGGER, NOCALIBRATEDATA, SINGLEIO.
		 *                  Note that BLOCKIO, BURSTIO, and SINGLEIO are mutually exclusive and that if
		 *                  BLOCKIO is specified, <code>numChannels*samplesPerChannel</code> must not exceed
		 *                  32768.
		 * @throws LibcbwException if an error occurs.
		...
		/**
		 * End an asynchronous scan, and return all samples that have yet to be processed.
		 * 
		 * @return a two-dimensional array indexed as <code>array[channel][sample]</code> of
		 *         16-bit unsigned measurements, zero-extended to fit <code>int</code>s, or 
		 ...
		 * @throws LibcbwException if an error occurs.
		 */
		public void analogueInToFile(String path, int lowchan, int numChannels, int range, int samplesPerChannel, int freq, int options) throws LibcbwException {
			NativeLong lcount = new NativeLong(samplesPerChannel * numChannels);
			NativeLongByReference rateref = new NativeLongByReference(new NativeLong(freq * numChannels));
			
			int err = LibcbwJNA.cbFileAInScan(_boardnum, lowchan, lowchan + numChannels - 1, lcount, rateref, range, path, options);
			if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err); 
		}
		 ...
		 * @param above - <code>true</code> if the threshold is a lower limit, or <code>false</code> if it is an upper limit
		...
		public void waitForThreshold(int channel, boolean above, int threshold, int range) throws LibcbwException {
			int err = LibcbwJNA.cbATrig(_boardnum, channel, above ? TriggerType.TRIGABOVE : TriggerType.TRIGBELOW, (short) threshold, range, new ShortByReference());
			if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err); 
		}
		...
		public void waitForThreshold(int channel, boolean above, float threshold, int range) throws LibcbwException {
			...
			if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err); 
			err = LibcbwJNA.cbATrig(_boardnum, channel, above ? TriggerType.TRIGABOVE : TriggerType.TRIGBELOW, raw_level.getValue(), range, new ShortByReference());
			if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err); 
		}
		 ...
		 * 
		 * @param rising - <code>true</code> for a rising-edge trigger, or <code>false</code> for a falling-edge trigger.
		 * @throws LibcbwException if an error occurs.
		 */
		public void setTrigger(boolean rising) throws LibcbwException {
			int err = LibcbwJNA.cbSetTrigger(_boardnum, rising ? TriggerType.TRIG_POS_EDGE : TriggerType.TRIG_NEG_EDGE, (short)0, (short)0);
			if(err != ErrorCode.NOERRORS) throw LibcbwException.fromErrorCode(err); 
		}
		 ...
		 * 
		 * @return the logic values of the DIO lines taken together as a <code>byte</code>.
		 * @throws LibcbwException if an error occurs.
		 */
		 ...
		 * 
		 * @param value - a <code>byte</code> with each bit representing the value to write to a
		 *                different DIO line.
		 * @throws LibcbwException if an error occurs.
		 */
		 ...
		 * 
		 * @param type - the type of event to handle. Must be one of the following constants 
		 *               (declared in {@link LibcbwJNA.EventType}):
		...
		/**
		 * Deregisters any event handler for the specified interrupt type(s) on this board.
		 * 
		 * @param type - the type of event to stop handling. Must be one of the following constants 
		 *               (declared in {@link LibcbwJNA.EventType}):
		 *               ON_SCAN_ERROR, ON_DATA_AVAILABLE, ON_END_OF_INPUT_SCAN, or ALL_EVENT_TYPES
		 * @throws LibcbwException if an error occurs.
		 */
		 ...
		 * 
		 * @return the current count value as a 32-bit unsigned integer, zero extended to fit a <code>long</code>
		 * @throws LibcbwException if an error occurs.
		 */
