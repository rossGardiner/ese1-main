...
public class LibcbwLogfile {
	...
	public LibcbwLogfile(String filename) throws LibcbwException {
		...
		_start = LocalDateTime.of(date >> 16, (date & 0xFF00) >> 8, date & 0xFF, hour, (time & 0xFF00) >> 8, time & 0xFF);
	...
	public LocalDateTime[] readTimeTags() throws LibcbwException {
		...
		for(int i = 0; i < _sample_count; i++) {
			...
			ret[i] = LocalDateTime.of(date >> 16, (date & 0xFF00) >> 8, date & 0xFF, hour, (time & 0xFF00) >> 8, time & 0xFF);
		}
		return ret;
	}

	public float[][] readAIChannels() throws LibcbwException {
		FloatBuffer buffer = ByteBuffer.allocateDirect(4 * _sample_count * _ain_count ).order(ByteOrder.nativeOrder()).asFloatBuffer();
	...
	public float[][] readCJCChannels() throws LibcbwException {
		FloatBuffer buffer = ByteBuffer.allocateDirect(4 * _sample_count * _cjc_count ).order(ByteOrder.nativeOrder()).asFloatBuffer();
	...
	public int[][] readDIOChannels() throws LibcbwException {
		IntBuffer buffer = ByteBuffer.allocateDirect(4 * _sample_count * _dio_count ).order(ByteOrder.nativeOrder()).asIntBuffer();
